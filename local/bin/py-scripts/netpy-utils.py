...
def new_network_ips_table(hosts, og_subnets, new_masks):
    ...
    for i in range(len(hosts)):
        ...
        print("Number of subnets created:", 2**og_subnet_mask_bits)
        print("Number of old hosts bits per subnet:", 32 - og_subnet_mask_bits)
        print("Number of hosts per subnet:", (2**))
    ...
    if args.hostsfile is not None:
        if args.ogfile is not None:
            if args.newfile is not None:
                ...
                print("Error. Some of the input files were not define passed.For more information use netpy-tuls -h")
                exit()
        else:
            print("Error. Some of the input files were not define passed. For more information use netpy-tuls -h")
            exit()
